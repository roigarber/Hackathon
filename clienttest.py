...
def perform_udp_transfer(server_ip, server_udp_port, file_size, transfer_id, stats_lock, stats):
    ...
    Now, the file_size is interpreted as the total number of bytes to be received.
    The client receives packets (each with a small header plus a data payload)
    and accumulates the total bytes.
    """
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_sock:
            ...
            # Continue receiving until we have accumulated file_size bytes or timeout occurs
            while total_received_bytes < file_size:
                try:
                    data, addr = udp_sock.recvfrom(2048)
                    current_time = time.time()
                    if len(data) < 9:
                        continue  # header is 9 bytes long
                    header = data[:9]
                    payload = data[9:]
            ...
            colored_print(
                ...
                f"total speed: {speed:.2f} bits/second, total bytes received: {total_received_bytes}",
                Colors.OKCYAN)
            with stats_lock:
